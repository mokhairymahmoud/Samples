...
```bash
curl http://localhost:29193/management/v4/transferprocesses/<transfer process id> | jq
```
...
> **Note**: The `/v3/edrs/` endpoint is deprecated as of EDC 0.17.0 and will be removed in a future release.
> It is retained here because no `/v4/edrs/` replacement exists yet; the EDR cache is being superseded by
> the Data Plane Signaling (DPS) flow in a future EDC version.
