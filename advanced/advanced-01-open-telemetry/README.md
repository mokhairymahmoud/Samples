...
curl -H "X-Api-Key: password" \
  ...
  -X POST "http://localhost:29193/management/v4/catalog/dataset/request" \
  -s | jq
```
...
curl -H "X-Api-Key: password" \
  ...
  -X POST "http://localhost:29193/management/v4/contractnegotiations" \
  -s | jq
```
...
```shell
curl -H 'X-Api-Key: password' \
  -X GET "http://localhost:29193/management/v4/contractnegotiations/{{contract-negotiation-id}}" \
  -s | jq
```
...
curl -H "X-Api-Key: password" \
  ...
  -X POST "http://localhost:29193/management/v4/transferprocesses" \
  -s | jq
```
